...
# TODO: Переделать режимы в словарь или список.
# TODO: Добавить возможность управления таймером с клавиатуры.
# TODO: Продумать возможность добавления новых режимов работы без того чтобы влезать в код.
...
class MainWindow(Gtk.Window):   # класс основного окна с тремя таймерами
    ...
    def draw_text(self, text, size, coord_x, coord_y, cr, color=(1, 1, 1)):   # функция для отрисовки одной строки текста
        ...
        (x, y, textWidth, textHeight, dx, dy) = cr.text_extents(text)
        cr.move_to(coord_x - dx / 2, coord_y + textHeight / 2)
        '''
        Пояснение: text_extents возвращает параметры того, сколько будет занимать на экране текст, если его выводить
        функцией show_text. dx, dy - на сколько сместится курсор по оси X, и Y соответственно, textWidth, textHeight - 
        ширина и высота закрашиваемых пикселей (это важно). Если подсунуть в текст одни пробелы - textWidth и textHeight
        будут 0, но dx - нет. Тоже касается выбранного шрифта Digital Dismay, если печатать им цифру 1 значения dx и 
        textWidth будут разными. Это приводит к сдвигам текста когда пытаемся вывести на экран 10:00 и 09:59. 
        Поэтому мы смотрим на смещение курсора и на высоту текста при печати. (Скорее всего я написал довольно невнятно,
         поэтому вот пример:
        (x, y, textWidth, textHeight, dx, dy) = cr.text_extents("10:00")
        print("textWidth = %d, textHeight = %d, dx = %d, dy = %d" % (textWidth, textHeight, dx, dy))
        (x, y, textWidth, textHeight, dx, dy) = cr.text_extents("09:59")
        print("textWidth = %d, textHeight = %d, dx = %d, dy = %d" % (textWidth, textHeight, dx, dy))
        '''
        cr.show_text(text)
...
mainTimer = TimerClass([[10, 2], [0, 10]], 'main')   # тут главный
